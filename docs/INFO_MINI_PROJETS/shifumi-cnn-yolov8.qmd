...
- **Précision sur les données d'entraînement et de validation** : après quelques epochs, le modèle atteint une bonne Précision (`accuracy rate`) (99%), ce qui indique qu’il a bien appris à différencier les classes sur mes images.
...
plt.title("Évolution de l'Précision et de la perte sur les données de validation")
plt.xlabel('Époques')
plt.ylabel('Valeur')
plt.grid(True)
plt.legend()
