...
```
- `Log-loss train (0.1900)` et `log-loss test (0.1796)` sont très proches, et **la perte est même légèrement meilleure (plus faible) sur le test**.
...
- Les valeurs observées (log-loss de 0,1900 au train et 0,1796 au test) sont très proches, voire légèrement meilleures sur le test.  
...
Le choix entre ces deux approches dépendra donc principalement :  
- De la nécessité d’une meilleure interprétabilité du modèle et d’une performance optimale selon l’**AUC** (avantage régression logistique),  
- Ou de la recherche d’une **précision globale légèrement supérieure** (avantage KNN).
...
- Le `KNN` excelle particulièrement lorsque les relations entre variables sont complexes et non linéaires;  
