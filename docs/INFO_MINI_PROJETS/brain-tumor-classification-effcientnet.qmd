...
Accurate classification of brain tumors from magnetic resonance imaging (MRI) is essential for guiding therapeutic decisions and improving patient outcomes. In this study, we propose a deep learning approach based on transfer learning with the pre-trained `EfficientNetB5 convolutional neural network`. A dataset of `6 056` T1-weighted MR images comprising `gliomas` (2 004 images), `meningiomas` (2 004 images), and `other tumor types` (2 048 images) was split into `training (70 %)`, `validation (15 %)`, and `test (15 %)` sets. Images were resized to `224 × 224 pixels`, `normalized`, and `augmented` through random rotations (±15°), zooms (±20 %), and horizontal flips. EfficientNetB5’s convolutional base was frozen up to layer 95, and a custom classifier head (GlobalAveragePooling2D → Dense(128, ReLU) → Dense(3, softmax)) was fine-tuned using the Adam optimizer (learning rate 1 × 10⁻⁵) with early stopping on validation loss.

On the independent test set (907 images), our fine-tuned EfficientNetB5 achieved an `overall accuracy of 99.67 %`, with only three misclassifications (`3/907`). Per-class metrics were as follows:

- `Glioma`: **Precision 99.34 %**, **Recall 100 %**, **F1-score 99.67 %** (support 300)

- `Meningioma`: **Precision 100 %**, **Recall 99.00 %**, **F1-score 99.50 %** (support 300)

- `Other tumors`: **Precision 99.68 %**, **Recall 100 %**, **F1-score 99.84 %** (support 307)

Macro-averaged and weighted-averaged precision, recall, and F1-scores all reached 99.67 %, confirming the model’s excellent discriminative ability and generalization on unseen MRI data. These performance levels closely corroborate results reported in the literature for transfer-learning-based brain tumor classification, where accuracies typically exceed 98 % under similar experimental conditions.

---

## Résumé

La classification précise des tumeurs cérébrales à partir d’images par résonance magnétique (IRM) est cruciale pour orienter les décisions thérapeutiques et améliorer le pronostic des patients. Dans cette étude, nous développons une approche d’apprentissage profond basée sur le transfert de connaissances avec le `réseau EfficientNetB5 pré-entraîné`. Un jeu de données de `6 056` images IRM T1—composé de `gliomes` (2 004 images), `méningiomes` (2 004 images) et autres types de tumeurs (2 048 images) a été réparti en `ensembles d’entraînement (70 %)`, de `validation (15 %)` et de `test (15 %)`. Les images ont été redimensionnées à `224 × 224 pixels`, `normalisées` et `enrichies` par des rotations aléatoires (±15°), zooms (±20 %) et flips horizontaux. Les couches convolutionnelles jusqu’à la couche 95 d’EfficientNetB5 ont été gelées, puis le modèle a été affiné avec une tête de classification (GlobalAveragePooling2D → Dense(128, ReLU) → Dense(3, softmax)), entraînée avec l’optimiseur Adam (taux d’apprentissage 1 × 10⁻⁵) et un arrêt précoce basé sur la perte de validation.

Sur l’ensemble de test indépendant (907 images), notre modèle a atteint une `exactitude globale de 99,67 %`, avec seulement trois erreurs de classification. Les métriques par classe sont :

- `Gliome` : **Précision 99,34 %**, **Rappel 100 %**, **F1-score 99,67 %** (support 300)

- `Méningiome` : **Précision 100 %**, **Rappel 99,00 %**, **F1-score 99,50 %** (support 300)

- `Autres tumeurs` : **Précision 99,68 %**, **Rappel 100 %**, **F1-score 99,84 %** (support 307)

Les moyennes macro et pondérées pour la précision, le rappel et le F1-score sont toutes à 99,67 %, confirmant la robustesse et la capacité de généralisation du modèle. Ces résultats corroborent ceux de la littérature, où des approches similaires atteignent généralement des précisions supérieures à 98 % pour la classification multiclasse des tumeurs cérébrales.
