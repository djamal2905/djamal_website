...
On the independent test set (907 images), our fine-tuned EfficientNetB5 achieved an `overall accuracy of 99.67 %`, with only three misclassifications (`2/906`). Per-class metrics were as follows:
...
Sur l'ensemble de test indépendant (907 images), notre EfficientNetB5 affiné a atteint une "précision globale de 99,67 %", avec seulement trois erreurs de classification (`2/906`). Les mesures par classe sont les suivantes :
