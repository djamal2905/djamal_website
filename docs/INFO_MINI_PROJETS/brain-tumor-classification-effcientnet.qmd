...
- `Accuracy (exactitude)` sur les données d’entraînement : `96,33 %`
...
- `Validation accuracy (exactitude sur les données de validation)` : `95,22 %`
...
- La très haute performance obtenue peut être en partie liée à la taille importante du jeu de données, qui a permis un apprentissage plus robuste. En effet, vous avez mentionné avoir environ deux fois plus d’images que dans certaines études comparables.  
...
## Annexe 3 : Evolution de l'exactitude de prédiction et de la fonction de perte de la validation au cours de l'entraînement du modèle



---

## Annexe 4: Détails de calculs des métriques
