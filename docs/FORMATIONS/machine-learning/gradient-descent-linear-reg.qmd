...
# Génération des itérations
np.random.seed(42)
iterations = np.arange(0, 100)

# Simulations de comportements de coût
...
plt.ylabel("Coût simulé")
plt.title("Différents comportements de la fonction de coût", fontsize=12)
plt.legend()
plt.tight_layout()
plt.show()
...
def gradient_descent(
    ...
    for i in range(max_iter):
        ...
        if i % 15000 == 0: # vous pouvez ajuster: moi je ne veux pas afficher toutes les itérations
            print(
...
df_5_r = py$df_5
display_table(df_5_r, "Les premières lignes de notre base de données", nrow_ = 5)
```
...
df.describe() # affcihe le résumé statistique des variables numériques
```
...
|       De prime abord on pour se dire qu'il existe un lien non linéaire entre les variables, ce qui n'est pas totalement juste car ... 
...
# predictions standardisé
ypred_scaled = predict(best['w'], best['b'], X_processed).reshape((-1, 1))
print(f'Données standardisées (3 premières valeurs): {ypred_scaled[:4]}\n\n')

# echelle normale
y_pred = scaler_y.inverse_transform(ypred_scaled)
print(f'Données avec leur échelle normale (3 premières valeurs): {y_pred[:4]}\n\n')
...
#| label: kfold-illustration
#| fig-cap: Illustration du processus de la validation croisée
...
- Le modèle a été testé sur 5 sous-ensembles différents, ce qui donne une estimation plus robuste de sa performance.

- La **RMSE moyenne de 0.15442** signifie que, **en moyenne, les prédictions s’écartent de la vérité d’environ 0.15 unité sur l’échelle standardisée**.

- La **faible variabilité** des scores suggère que le modèle reste globalement stable.

### Visualisation des erreurs par pli

```{python, fig.align='center', echo= TRUE, out.width='90%', out.height='80%'}
#| label: rmse-value-plot
#| fig-cap: RMSE par pli (validation croisée 5-fold)
#| code-fold: true
#| code-fold-show: true
...
Maintenant calculons le `RMSE` des erreurs commises en prédisant. D'abord essayons de visualiser cela.
D'abord de manière séparée car il me vient à l'idée d'ordonner les différents arrays obtenus, mais avant faut que je sois sûre que les salaires réels (pas dans le sens économique du terme mais pour dire salaire observé) et ceux prédits ont la même allure.

```{python, fig.align='center', echo= TRUE, out.width='90%', out.height='80%'}
#| label: testing-final plot
...
```{python, fig.align='center', echo= TRUE, out.width='90%', out.height='80%'}
#| label: testing-final-sorted plot
...
# RMSE sur les salaires standardisés

# RMSE sur les salaires réels
...
la différence entre le minimum et le maximum est d’environ 60 000 €.
...
L’analyse menée à travers la génération, la modélisation et la validation croisée de notre dataset salarial montre que le modèle développé est performant et robuste.

- La **validation croisée en 5 plis** a permis d’évaluer la stabilité du modèle, avec une **RMSE moyenne standardisée autour de 0.15**, ce qui indique une bonne capacité de généralisation sur des données non vues.

- En ramenant la RMSE à l’échelle réelle des salaires, on obtient une erreur moyenne d’environ **284 €**, soit **0,56 %** de la moyenne salariale. Cette précision relative est très satisfaisante compte tenu de la variabilité naturelle des salaires.

- La convergence rapide de l’algorithme de descente de gradient, testée sur plusieurs valeurs du taux d’apprentissage, montre que le modèle est bien paramétré et que la fonction de coût est efficacement minimisée.

- Il est important de noter que pour éviter le phénomène d’**overfitting**, plusieurs pistes peuvent être explorées dans de futurs travaux, telles que l’augmentation de la taille des données, une sélection judicieuse des variables explicatives, ou l’introduction de régularisations dans le modèle.

- Enfin, ce travail pose les bases solides pour des modèles plus avancés, notamment la régression logistique pour des problèmes de classification, ou des régressions polynomiales pour capter des relations non linéaires, qui seront explorés dans des publications futures.

En somme, ce projet illustre clairement l’importance d’une préparation rigoureuse des données, d’un choix approprié des hyperparamètres, et d’une validation systématique pour construire des modèles prédictifs fiables et interprétables. J'ai été ravis d'avoir passer 20 h à préparer et écrire cette publication, cela m'a permis de consolidé mes acquis en **Python**, **Machine Learning (Théorie+ pratique)** et en rigeur méthodologique.
