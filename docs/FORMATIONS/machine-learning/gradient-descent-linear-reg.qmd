...
- **💡 Interprétation :**
  ...
  - 📌 Une analyse plus fine (ex. tests statistiques) serait nécessaire pour confirmer l’absence d’écart significatif (mais ce n'est pas le but ici, [Cliquez ici pour voir une de mes publications ou je fais des tests statistics afin de selectionner les variables essentielles pour la spécification de mon modèle](https://djamal2905.github.io/djamal_website/INFO_MINI_PROJETS/classi_bin_acp_kmeans_knn_logit/work.html)).
...
Le fait que la `RMSE` sur la validation soit nettement plus élevée que celle obtenue lors de l'apprentissage :
...
⚖️ Déséquilibre dans les variables catégorielles : il se peut que la répartition des niveaux d’éducation ou des sexes soit différente entre le train et la validation, ce qui fausse l’évaluation (Ce qui est plus probable car au debut j'avais mentionné ce soucis).
