...
# print("AUC test :", roc_auc_score(y_test, y_proba_knn))
```
...
df_knn <- data.frame(
  Modèle = rep("KNN", 6),
  Classe = c("Benin", "Malin", "Accuracy", "Macro avg", "Weighted avg", "AUC ROC"),
  Précision = c(0.95, 0.89, NA, 0.92, 0.93, 0.9742),
  Recall = c(0.93, 0.92, NA, 0.93, 0.93, NA),
  F1_Score = c(0.94, 0.91, 0.93, 0.93, 0.93, NA),
  Support = c(107, 64, 171, 171, 171, NA),
  stringsAsFactors = FALSE
)
...
df_model$diagnosis_num <- ifelse(df_model$diagnosis == "M", 1, 0)
probs <- fitted(model_logit_std_weighted)

# on évte d’avoir des prédictions exactement 0 ou 1
probs <- pmax(pmin(probs, 0.9999), 0.0001)

hoslem.test(df_model$diagnosis_num, probs, g=10)
```
