...
|       Ici le stepAIC permet de fournir une sélection de variables qui améliore le modèle (***critère d'AIC que j'aborderai dans une autre publication***).

```{r}
mod1_poisson <- step(model) 
```

Les variables c-dessus sont celles qui seront utilisées pour le modèle finale (poisson s'il est valide ou quasi-poisson ou binomial négatif s'il y'a présence de surdispersion dans les données).
...
```{r}
model_nb <- glm.nb(`Cas palustres` ~ Humidité + Pluviométrie + 
                     `Vitesse du vent`+ Phase, 
                   data = data)

#summary(model_nb, exponentiate = TRUE)
```
